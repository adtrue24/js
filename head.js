...
function img_error(el)
{
    ...
    if(ss[1])
    {
        ...
        // console.log(img_org, scheme, domain, path,ss2);
    }


    if(img_data_src == null)      { img_hide(el); return; }
    if(img_src == img_data_src)   { img_hide(el); return; }
    el.src = img_data_src;
...
{
    var rs = img_data_src;
    //rs = img_data_src.replace("https://", "https://cdn.tax/wp/");
    return rs;
}
...
function img_lazy(elements_selector)
{
    var lazyLoadInstance = new LazyLoad({
        ...
        threshold:300,
        load_delay:0,
        callback_loaded: function(el) { el.removeAttribute("data-src");},
        callback_reveal: function(el) {
          if (referrerPolicy_supported) { el.referrerPolicy = "no-referrer"; }
          var new_src =  img_proxy(el.getAttribute("data-src")); el.src= new_src;},
        });
}